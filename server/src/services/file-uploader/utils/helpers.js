...
const defineAbilitiesHelpers = {
  ...
  checkUserContext: function (payload) {
    ...
    try {
      ...
      hook.app.error('failed to checkUserContext');
    }
    return result;
  },
 ...
 * @example
 * app.service('posts').get('someId', callingParamsPersistUser(hook.params, {query: {'someKey': 'someValue'}}));
 */
const callingParamsPersistUser = function (params, newParams) {
  /**
   * until 1.8.4 the first attribute was hook object and not params
   */
  const isHook = params && params.path;
  if (isHook) {
    // eslint-disable-next-line no-console
    console.warn('callingParamsPersistUser function - please stop passing hook at first attribute, need to pass only the params');
  }
